...
bool readFileToMatrix(const std::string &filePath, Matrix &mat)
{
	...
	if (!in.good())
	{
		return false; // file not open or failed
	}

	// Attempt to read exactly rows*cols floats from file
	for (int i = 0; i < mat.get_rows() * mat.get_cols(); i++)
	{
		if (!in.read(reinterpret_cast<char*>(&mat[i]), sizeof(float)))
		{
			return false; // not enough data in file
		}
	}

	// Optionally, you can check if there's extra data or not.
	// But typically you can just ignore it for this assignment.
